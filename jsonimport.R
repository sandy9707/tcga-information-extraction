...
for (i in seq_along(filedir_in_json)) {
  setwd(paste0("./", filedir_in_json[i])) # 切换目录到 file_id 所在的目录
  matrix_MMRF[[i]] <- read.table(file = list.files(pattern = ".tsv"), header = F, fill = TRUE)[, 7] # 读取文件中的第7列作为表达矩阵，存储到列表中
  setwd("../") # 切换目录回上一级
}
